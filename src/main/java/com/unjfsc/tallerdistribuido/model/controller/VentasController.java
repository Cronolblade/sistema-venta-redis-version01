...
public class VentasController {
    ...
    public VentasController(
            @Qualifier("redisReplicaTemplate") StringRedisTemplate redisReplicaTemplate,
    ...
    private void verifyKeyReplication(String key, boolean shouldExist) {
        ...
        for (int i = 0; i < 25; i++) {
            try {
                ...
                System.out.println("Intento " + (i + 1) + ": Verificando existencia de '" + key + "'. ¿Debería existir? " + shouldExist + ". ¿Existe en la réplica? " + keyExistsInReplica);

                if (keyExistsInReplica != null && keyExistsInReplica == shouldExist) {
                    System.out.println("INFO: Replicación de existencia de clave '" + key + "' verificada en el intento " + (i + 1));
                    return;
                }
        ...
        System.err.println("WARN: No se pudo verificar la replicación de existencia para la clave " + key + " después de 25 intentos.");
    }
    ...
    private void verifyStockReplication(String key, int expectedStock) {
        for (int i = 0; i < 25; i++) {
            try {
                ...
                System.out.println("Intento " + (i + 1) + ": Verificando stock de '" + key + "'. Stock esperado: " + expectedStock + ". Stock en réplica: " + (stockStr != null ? stockStr : "null"));

                if (stockStr != null) {
                    int replicaStock = Integer.parseInt(stockStr);
                    if (replicaStock == expectedStock) {
                        System.out.println("INFO: Replicación de stock para '" + key + "' verificada con valor " + expectedStock + " en el intento " + (i+1));
                        return;
                    }
                }
                TimeUnit.MILLISECONDS.sleep(200);
        ...
        System.err.println("WARN: No se pudo verificar la replicación del stock para la clave " + key + " después de 25 intentos.");
    }
